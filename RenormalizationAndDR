...
seu <- ScaleData(seu,  features=rownames(seu),vars.to.regress = "percent.mt", block.size = 500, min.cells.to.block = 1000)
plan("sequential")

seu <- RunPCA(seu,  npcs=50, features=VariableFeatures(object=seu),  verbose = FALSE)
# seu <- RunUMAP(object=seu,  dims=1:30, verbose=FALSE, min.dist=0.5, n.neighbors = 30L) # uwot in seurat3.1 is problematic
seu <- RunUMAP(object=seu,  dims=1:30, verbose=FALSE, min.dist=0.5, n.neighbors = 30L, umap.method = "umap-learn", metric = "correlation")
seu <- RunTSNE(object=seu,  dims=1:30, verbose=FALSE, perplexity=60)
toc()
